...
- **`maxFrontOverlapAtSpeed` can be negative** (speed too high for capture rate at current footprint). Agent 1 leaves it raw on purpose — no clamp in the core. The ceiling readout must not print a negative number: clamp display at 0 or treat any value ≤ 0 as out-of-spec; the verdict logic already keys off target > ceiling.
